...
def _legs_task():
    ...
    print(f"[{threading.current_thread().name}] legs done: {ok}")
...
def dance():
    ...
    legs_thread = threading.Thread(name="legs_thread", target=_run_with_event_loop, args=(_legs_task,))
    arms_thread = threading.Thread(name="arms_thread", target=_run_with_event_loop, args=(_arms_task,))
    legs_thread.start()
    arms_thread.start()
    legs_thread.join()
    arms_thread.join()
